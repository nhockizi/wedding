(function($){
	...
	$(document).ready(function() {
		...
		$('.intro-slider-bamboo').owlCarousel({
			paginationSpeed: 1200,
			pagination: true,
			navigation: false,
			singleItem: true,
		...
		$( "#emailsubmit" ).click(function() {
			var emailfrom = $('#semail').val();
			var emailto   = 'info@ooj-vietnam.com';
			var emailcc   = 'ralf@ooj-vietnam.com';
			var name      = $('#sname').val();
			var surname   = $('#ssurname').val();
			var phone     = $('#stelephone').val();
			var message   = $('#smessage').val();
			var interested= $('#sinterested').val();
			var subject   = 'Customer of OOJ';
			var contentInterest = 'Interested in: ' + interested;
			if(interested == ''){
				contentInterest = '';
			};
			var content   = '<p>'+ contentInterest + '<br>' + message +'</p>'
							+ 'Best regards, <br>' 
							+ name + ' ' + surname 
							+ '<br> Email: ' + emailfrom 
							+ '<br> Telephone: ' + phone;
			...
			$.ajax({
		            type: "post",
		            url: "/cms-services/helper.cfc?method=sendEmail",
		            data: {
		            pemailfrom : emailfrom,
		            pname : name,
		            psubject : subject,
		            pmessagecontent : content,
		            pemailto : emailto,
		            pemailcc : emailcc
		           },
		           cache: false,
		           ...
		           success: function (data) {
			           	if (data == true) {
			           		...
			           		alert('An occurred fails with your request !');
			           	}
		           },
		           error: function () {
		            alert('An occurred fails with your request !');
		   ...
		   setInterval(function () {$('.actionloader').addClass('hide')}, 3000);
		});
